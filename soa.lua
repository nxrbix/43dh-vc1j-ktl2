...
utility.floatlist = function(list, anchor, height, host)
	local hostPos = host.AbsolutePosition
	local aPos = anchor.AbsolutePosition
	local aSize = anchor.AbsoluteSize
	local w = aSize.X
	local h = height
	local x = aPos.X - hostPos.X
	local y = aPos.Y - hostPos.Y + aSize.Y + 1
	if y + h > host.AbsoluteSize.Y - 2 then
		local up = aPos.Y - hostPos.Y - h - 1
		if up >= 2 then
			y = up
		end
	end
	...
	for _, d in ipairs(list:GetDescendants()) do
		if d:IsA("GuiObject") then
			d.ZIndex = math.max(d.ZIndex, 251)
		end
	end
	list.Parent = host
	list.Visible = true
end
utility.docklist = function(list, dock, height)
	list.Visible = false
...
function library:new(props)
	...
	--
	local overlay = utility.new(
		"ScreenGui",
		{
			Name = tostring(math.random(0,999999)).."overlay",
			DisplayOrder = 100000,
			IgnoreGuiInset = true,
			ResetOnSpawn = false,
			ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
		...
		if v ~= ignore then
			if v.open then
				if v.hide then
					...
					if v.indicator then v.indicator.Text = "+" end
					v.open = false
				end
			end
		end
	...
	for i,v in pairs(window.colorpickers) do
		if v ~= ignore then
			if v.open then
				...
				if v.dockParent then
					v.cpholder.Size = UDim2.new(1,0,0,230)
					v.cpholder.Position = UDim2.new(0,0,1,5)
					v.cpholder.Parent = v.dockParent
				end
				v.open = false
			end
		end
...
function sections:dropdown(props)
	...
	local visibleRows = math.clamp(math.max(#options, 1), 1, max)
	local listHeight = 18 * visibleRows + 6
	...
	local optionsholder = utility.new(
		"Frame",
		{
			...
			BorderColor3 = Color3.fromRGB(130, 210, 220),
			BorderMode = "Outline",
			BorderSizePixel = 1,
	...
	for _, v in ipairs(options) do
		...
		ddoptionbutton.MouseButton1Down:Connect(function()
			...
			dropdown.current = v
			dropdown.value.Text = tostring(v)
			ddoptiontitle.TextColor3 = self.library.theme.accent
	...
	dropdownbutton.MouseButton1Down:Connect(function()
		...
		for _, lab in pairs(dropdown.titles) do
			if lab.Text == tostring(dropdown.current) then
				lab.TextColor3 = dropdown.library.theme.accent
			else
...
function dropdowns:Refresh(newOptions)
	...
	max = math.clamp(tonumber(max) or 12, 1, 30)
	local visibleRows = math.clamp(math.max(#newOptions, 1), 1, max)
	local listHeight = (18 * visibleRows) + 6
	dropdown.listHeight = listHeight

	if dropdown.optionsholder and not dropdown.open then
		dropdown.optionsholder.Size = UDim2.new(1, 0, 0, listHeight)
		dropdown.optionsholder.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
		dropdown.optionsholder.BackgroundTransparency = 0
		dropdown.optionsholder.BorderColor3 = Color3.fromRGB(130, 210, 220)
		dropdown.optionsholder.ZIndex = 50
	elseif dropdown.optionsholder then
		dropdown.optionsholder.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
		dropdown.optionsholder.BackgroundTransparency = 0
		dropdown.optionsholder.BorderColor3 = Color3.fromRGB(130, 210, 220)
	end
	if dropdown.optionsoutline then
		...
		dropdown.optionsoutline.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
		dropdown.optionsoutline.BackgroundTransparency = 0
		dropdown.optionsoutline.ZIndex = 51
		dropdown.optionsoutline.ScrollBarImageColor3 = Color3.fromRGB(255, 255, 255)
		dropdown.optionsoutline.ScrollBarThickness = 4
	end
	...
	for _, v in ipairs(newOptions) do
		...
		table.insert(dropdown.titles, ddoptiontitle)
		table.insert(dropdown.buttons, ddoptionbutton)
		if tostring(v) == tostring(dropdown.current) then
	...
	local still = false
	for _, v in ipairs(newOptions) do
		if tostring(v) == tostring(dropdown.current) then still = true break end
	end
	if not still and #newOptions > 0 then
		dropdown.current = newOptions[1]
		dropdown.value.Text = tostring(newOptions[1])
...
--
function dropdowns:show()
	local dropdown = self
	local host = dropdown.library.outline
	local anchor = dropdown.anchor or dropdown.dropdownholder
	if not dropdown.optionsholder or not host or not anchor then return end
	utility.floatlist(dropdown.optionsholder, anchor, dropdown.listHeight, host)
	dropdown.open = true
	if dropdown.indicator then dropdown.indicator.Text = "-" end
	if dropdown.floatConn then
		dropdown.floatConn:Disconnect()
		dropdown.floatConn = nil
	end
	dropdown.floatConn = rs.Heartbeat:Connect(function()
		if not dropdown.open then return end
		local a = dropdown.anchor or dropdown.dropdownholder
		local h = dropdown.library.outline
		if a and h and dropdown.optionsholder then
			utility.floatlist(dropdown.optionsholder, a, dropdown.listHeight, h)
		end
	end)
end
--
function dropdowns:hide()
	local dropdown = self
	if dropdown.floatConn then
		dropdown.floatConn:Disconnect()
		dropdown.floatConn = nil
	end
	if not dropdown.optionsholder then return end
	utility.docklist(dropdown.optionsholder, dropdown.dockParent or dropdown.dropdownholder, dropdown.listHeight or 20)
	dropdown.open = false
	if dropdown.indicator then dropdown.indicator.Text = "+" end
end
--
...
function sections:colorpicker(props)
	...
	button.MouseButton1Down:Connect(function()
		...
		if colorpicker.open then
			...
			cpholder.Size = UDim2.new(1,0,0,230)
			cpholder.Position = UDim2.new(0,0,1,5)
			cpholder.Parent = colorpickerholder
			colorpicker.open = false
		else
			local abs = colorpickerholder.AbsolutePosition
			local size = colorpickerholder.AbsoluteSize
			local w = math.max(size.X, 180)
			local h = 230
			local x = abs.X
			local y = abs.Y + size.Y + 5
			...
			if x < 0 then x = 0 end
			if y < 0 then y = 0 end
			cpholder.Size = UDim2.fromOffset(w, h)
			cpholder.Position = UDim2.fromOffset(x, y)
			cpholder.AnchorPoint = Vector2.new(0, 0)
			cpholder.ZIndex = 200
			for _, d in ipairs(cpholder:GetDescendants()) do
				if d:IsA("GuiObject") then
					d.ZIndex = 201
				end
			end
			cpholder.Parent = self.library.overlay or self.library.screen
...
-- Anti-shove disabled for character (was suspected of causing persistent push).
-- Only keep prediction-ball safety; do not strip forces or unanchor HRP on inject.
task.defer(function()
    local ball = workspace:FindFirstChild("SoA_RaycastPrediction")
    if ball and ball:IsA("BasePart") then
...
local function updateBindPanel()
    ...
    if #lines == 0 then
        bindPanel:setlines({ "No active binds" })
    else
        bindPanel:setlines(lines)
    end
end
...
local function updateGunVFX()
    ...
    if currentGunVFX then
        ...
        currentGunVFX.CanCollide = false
        currentGunVFX.CanTouch = false
        currentGunVFX.CanQuery = false
        for _, d in ipairs(currentGunVFX:GetDescendants()) do
            if d:IsA("BasePart") then
                d.CanCollide = false
                d.CanTouch = false
                d.CanQuery = false
...
local function releaseThirdPersonCamera()
    if soaOwnsCamera then
...
do
    ...
    local function neutralizeViewmodelPart(obj)
        if not obj or not obj:IsA("BasePart") then return end
        -- Viewmodel must never collide with the local character (causes constant shove)
        obj.CanCollide = false
        obj.CanTouch = false
        obj.CanQuery = false
        obj.Massless = true
    ...
    getRigs().DescendantAdded:Connect(function(obj)
        if obj:IsA("BasePart") then
            obj.CanCollide = false
            obj.CanTouch = false
            obj.CanQuery = false
        ...
        task.delay(0.1, function() updateViewmodel(false) end)
    end)
    -- Neutralize current rig immediately
    local rigsNow = getRigs()
    if rigsNow then
        for _, obj in ipairs(rigsNow:GetDescendants()) do
            if obj:IsA("BasePart") then
                obj.CanCollide = false
                obj.CanTouch = false
                obj.CanQuery = false
...
do
    ...
    local function updateAmbience()
        if not Settings.Ambience.Enabled then return end
    ...
    _G.SoA_RestoreAmbience = restoreAmbience

    ambienceSection:toggle({name = "Enabled", def = false, callback = function(v)
        Settings.Ambience.Enabled = v
        if v then updateAmbience() else restoreAmbience() end
    end})
...
do
    ...
    local function ensureColorCorrection()
        ...
        local existing = Lighting:FindFirstChild("SoA_ColorCorrection")
    ...
    local function updateColorCorrection()
        local fx = ensureColorCorrection()
        if not fx then return end
        fx.Enabled = Settings.ColorCorrection.Enabled
        fx.Contrast = Settings.ColorCorrection.Contrast
        fx.Saturation = Settings.ColorCorrection.Saturation
        fx.Brightness = Settings.ColorCorrection.Brightness
    end

    ensureColorCorrection()
    _G.SoA_UpdateColorCorrection = updateColorCorrection
    _G.SoA_EnsureColorCorrection = function()
        ensureColorCorrection()
...
LocalPlayer.CharacterAdded:Connect(function()
    ...
    task.delay(0.15, function()
        ...
        if _G.SoA_EnsureColorCorrection then
            _G.SoA_EnsureColorCorrection()
        end
    ...
    task.delay(0.5, invalidateShootCache)
